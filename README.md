...
_(Note: The Dockerfile uses `python:3.11` and installs `proj-bin` to ensure the `pyproj` geospatial library compiles correctly)._
...
- **Payload (FOV Method)**:
  _(Reference: `camera_input_fov.json`)_
...
- **Payload**:
  ...
  ```json
  {
    "start_point": [44.25, 28.35, 5],
    "end_point": [44.32, 28.60, 60],
    "offset_level": 1,
    "offset_step": 5.0,
